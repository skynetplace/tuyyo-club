...
// .wrangler/tmp/bundle-431Goy/strip-cf-connecting-ip-header.js
...
function formatBookingTelegramMessage(booking) {
  return `\u{1F3C4} <b>NEW BOOKING</b>
\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501
\u{1F4CB} Code: <b>${booking.booking_code}</b>
\u{1F464} Client: ${booking.full_name}
\u{1F4DE} Phone: ${booking.phone}
\u{1F4E7} Email: ${booking.email || "\u2014"}
\u{1F4CD} Location: ${booking.location_name}
\u{1F4C5} Start: ${booking.start_time}
\u{1F4B0} Total: \u20AC${booking.total_amount}
\u{1F4B3} Payment: ${booking.payment_status}`;
}
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: #40E0D0; padding: 20px; text-align: center;">
        <h1 style="color: #003366; margin: 0;">\u{1F3C4} ${siteName}</h1>
      </div>
      ...
      <div style="background: #f5f5f5; padding: 15px; text-align: center; color: #999; font-size: 12px;">
        \xA9 ${(/* @__PURE__ */ new Date()).getFullYear()} ${siteName} \xB7 Costa Blanca
      </div>
    </div>
  `;
  return { subject, html };
}
async function sendNotifications(env2, bookingId) {
  try {
    const booking = await env2.DB.prepare(`
    ...
    const settings = await getNotificationSettings(env2.DB);
    const promises = [];
    if (settings.notify_telegram_enabled === "1" && settings.telegram_bot_token && settings.telegram_chat_id) {
      ...
      promises.push(
        sendTelegram(settings.telegram_bot_token, settings.telegram_chat_id, text).then((ok) => console.log(`Telegram notification ${ok ? "sent" : "failed"} for booking ${bookingId}`))
      );
    }
...
async function sendPaymentConfirmation(env2, bookingId) {
  try {
    const booking = await env2.DB.prepare(`
      SELECT b.*, c.full_name, c.phone, c.email, l.name as location_name
      FROM bookings b
    ...
    if (!booking || !booking.email)
      return;
    const settings = await getNotificationSettings(env2.DB);
    if (settings.notify_email_enabled !== "1" || !settings.resend_api_key)
      return;
...
var init_notify = __esm({
  "src/lib/notify.js"() {
    ...
    __name(formatBookingTelegramMessage, "formatBookingTelegramMessage");
    __name(formatBookingEmail, "formatBookingEmail");
    __name(sendNotifications, "sendNotifications");
    __name(sendPaymentConfirmation, "sendPaymentConfirmation");
  }
});

// .wrangler/tmp/bundle-431Goy/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-431Goy/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
publicRoutes.get("/availability", async (c) => {
  ...
  const endDateTime = new Date(new Date(startDateTime).getTime() + durationHours * 3600 * 1e3).toISOString().slice(0, 19).replace("T", " ");
...
__name(getLocationById, "getLocationById");
async function getInventoryById(db, id) {
  return db.prepare(
    "SELECT * FROM inventory WHERE id = ? AND is_active = 1"
  ).bind(id).first();
}
__name(getInventoryById, "getInventoryById");
function getPriceForDuration(inventory, durationType) {
  const priceKey = {
    "2h": "price_2h",
...
__name(getPriceForDuration, "getPriceForDuration");
async function checkAvailability(db, inventoryId, startTime, endTime) {
  const inv = await db.prepare(
    "SELECT total_units FROM inventory WHERE id = ?"
  ).bind(inventoryId).first();
  if (!inv)
    return { available: 0, total_units: 0, booked: 0, exists: false };
  const booked = await db.prepare(`
    SELECT COALESCE(SUM(bi.quantity), 0) as total_booked
    FROM booking_items bi
    JOIN bookings b ON bi.booking_id = b.id
...
async function getAllSettingsMasked(db) {
  const { results } = await db.prepare(
    "SELECT key, value, is_secret FROM settings"
  ).all();
  const settings = {};
  results.forEach((r) => {
    settings[r.key] = r.is_secret && r.value ? "\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022" + r.value.slice(-4) : r.value;
  });
  return settings;
}
__name(getAllSettingsMasked, "getAllSettingsMasked");
...
async function createBooking(db, bookingData) {
  const res = await db.prepare(`
...
async function addBookingItem(db, bookingId, inventoryId, quantity, priceAtBooking) {
  await db.prepare(`
...
__name(addBookingItem, "addBookingItem");
async function setBookingStripeSession(db, bookingId, sessionId) {
  await db.prepare(
    "UPDATE bookings SET stripe_session_id = ? WHERE id = ?"
  ).bind(sessionId, bookingId).run();
}
__name(setBookingStripeSession, "setBookingStripeSession");
async function getBookingsList(db, limit = 100) {
  const { results } = await db.prepare(`
    SELECT b.*, c.full_name, c.phone, c.email, l.name as location_name
    FROM bookings b
    JOIN customers c ON b.customer_id = c.id
...
__name(getBookingsList, "getBookingsList");
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var _stripeModule = null;
async function getStripeModule() {
  if (!_stripeModule) {
    try {
      _stripeModule = await Promise.resolve().then(() => (init_stripe_esm_worker(), stripe_esm_worker_exports));
    } catch (err) {
...
__name(getStripeModule, "getStripeModule");
async function getClient(env2) {
  const Stripe2 = await getStripeModule();
  const stripeKey = env2.STRIPE_SECRET_KEY;
  if (!stripeKey) {
    throw new Error("STRIPE_SECRET_KEY is not configured");
  }
  return new Stripe2(stripeKey);
}
__name(getClient, "getClient");
async function createCheckoutSession(env2, { bookingCode, totalAmount, customerEmail, bookingId }) {
  const stripe = await getClient(env2);
  const session = await stripe.checkout.sessions.create({
    ...
    mode: "payment",
    success_url: `${env2.FRONTEND_URL}/success.html?code=${bookingCode}`,
    cancel_url: `${env2.FRONTEND_URL}/booking.html?canceled=1`,
...
async function handleStripeWebhook(env2, body, signature) {
  const Stripe2 = await getStripeModule();
    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        if (bookingId) {
          ...
          const { sendNotifications: sendNotifications2 } = await Promise.resolve().then(() => (init_notify(), notify_exports));
          if (sendNotifications2) {
            env2.executionCtx?.waitUntil?.(
              sendNotifications2(env2, parseInt(bookingId))
            );
          }
        }
        break;
...
bookingRoutes.get("/step-1", async (c) => {
  ...
  const timeOptions = Array.from({ length: 13 }, (_, i) => i + 7).map((h) => `<option value="${String(h).padStart(2, "0")}:00">${String(h).padStart(2, "0")}:00</option>`).join("");
  return c.html(`
...
bookingRoutes.post("/create", async (c) => {
  ...
  const hoursUntilStart = (startTime - now) / (1e3 * 60 * 60);
...
adminRoutes.post("/inventory", async (c) => {
  ...
  `).bind(
    ...
    data.price_2h || 35,
    data.price_half_day || 40,
    data.price_full_day || 50,
    data.price_multi_day || 40,
    data.deposit_amount || 150,
    data.total_units || 1,
    data.image_url || null,
...
app.onError((err, c) => {
  ...
  const message = status >= 500 && !isDev ? "Internal Server Error" : err.message || "Unknown error";
  return c.json({
    error: message,
